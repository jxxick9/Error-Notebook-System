...
* AI 協作： 引導 AI 生成 `User`、`ErrorItem` 及 `SystemManager` 的類別結構（class schemas），並引入智慧指標（`std::shared_ptr`）與時間戳記工具（`std::chrono`）。
* 成果： 成功生成標準的標頭檔定義與程式碼骨架。



## Phase 2: Runtime Logic Debugging

* 遇到問題： 新註冊的帳號可以成功登出，但在同一個工作階段或新的工作階段中嘗試重新登入時，會跳出 `[!] Login failed`（登入失敗）。
...
2. 重寫 `SystemManager::saveGlobalUsers()` 內部的序列化引擎，以正確儲存原始的雜湊值（hashes），而非布林旗標。
